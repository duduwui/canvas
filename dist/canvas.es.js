...
class H {
  ...
  init() {
    ...
    t.textContent = N, this.shadowRoot.appendChild(t), this.uiContainer = document.createElement("div"), this.uiContainer.innerHTML = this.getHTMLTemplate(), this.shadowRoot.appendChild(this.uiContainer), this.badge = this.shadowRoot.querySelector(".canvas-badge"), this.hoverOverlay = this.shadowRoot.querySelector(".canvas-hover-overlay"), this.selectionOverlay = this.shadowRoot.querySelector(".canvas-selection-overlay"), this.inspector = this.shadowRoot.querySelector(".canvas-inspector"), this.modalOverlay = this.shadowRoot.querySelector(".canvas-modal-overlay"), this.toastContainer = this.shadowRoot.querySelector(".canvas-toast-container");
    const e = localStorage.getItem("canvas_inspector_dock") || "dock-right";
    this.inspector.classList.add(e), this.tooltip = document.createElement("div"), this.tooltip.className = "canvas-tooltip", this.shadowRoot.appendChild(this.tooltip), this.cacheInputs(), this.bindEvents();
  }
  getHTMLTemplate() {
    return `
      <!-- Badge -->
  ...
  cacheInputs() {
    this.shadowRoot.querySelectorAll("[data-style]").forEach((t) => {
      const e = t.dataset.style;
      this.inputs[e] || (this.inputs[e] = []), this.inputs[e].push(t);
    });
  }
  bindEvents() {
    ...
    }), this.shadowRoot.querySelectorAll(".section-header").forEach((r) => {
      r.addEventListener("click", () => {
        r.parentElement.classList.toggle("collapsed");
      });
    ...
    }), this.shadowRoot.querySelectorAll(".tab-btn").forEach((r) => {
      r.addEventListener("click", () => {
        this.shadowRoot.querySelectorAll(".tab-btn").forEach((b) => b.classList.remove("active")), this.shadowRoot.querySelectorAll(".tab-content").forEach((b) => b.classList.remove("active")), r.classList.add("active");
        const c = `tab-${r.dataset.tab}`, p = this.shadowRoot.getElementById(c);
        p && p.classList.add("active"), r.dataset.tab === "layers" && this.updateLayersTree();
      });
    }), this.shadowRoot.querySelectorAll(".swatch-btn").forEach((r) => {
      r.addEventListener("click", () => {
        const c = r.closest(".control-row");
        if (c) {
          const p = r.dataset.color, b = c.querySelector(".color-text-input"), v = c.querySelector(".color-native-input");
          b && (b.value = p, b.dispatchEvent(new Event("input", { bubbles: !0 }))), v && p !== "transparent" && (v.value = p);
        }
      });
    });
    const e = this.shadowRoot.getElementById("canvas-font-file-input"), o = this.shadowRoot.getElementById("canvas-font-file-list");
    e && e.addEventListener("change", (r) => {
      const c = Array.from(r.target.files);
      if (c.length === 0) {
        o.textContent = "No files chosen";
        return;
      }
      o.textContent = c.map((p) => p.name).join(", "), this.callbacks.onLocalFontUpload && this.callbacks.onLocalFontUpload(c);
    });
    const a = this.shadowRoot.querySelector(".canvas-quick-toolbar");
    a && a.addEventListener("click", (r) => {
      const c = r.target.closest(".quick-btn");
      if (!c) return;
      const p = c.dataset.action;
      this.callbacks.onQuickAction && this.selectedElement && this.callbacks.onQuickAction(p, this.selectedElement);
    });
    const s = this.shadowRoot.getElementById("canvas-global-font-input");
    s && s.addEventListener("input", (r) => {
      const c = r.target.value.trim();
      this.callbacks.onGlobalFontChange && this.callbacks.onGlobalFontChange(c);
    });
    const l = this.shadowRoot.getElementById("depth-btn-front"), n = this.shadowRoot.getElementById("depth-btn-forward"), d = this.shadowRoot.getElementById("depth-btn-backward"), f = this.shadowRoot.getElementById("depth-btn-back");
    l && l.addEventListener("click", () => {
      this.callbacks.onStyleChange && this.selectedElement && (this.callbacks.onStyleChange("position", "relative", this.selectedElement), this.callbacks.onStyleChange("zIndex", "9999", this.selectedElement), this.showToast("Brought item to absolute front", "success"));
    }), n && n.addEventListener("click", () => {
      if (this.callbacks.onStyleChange && this.selectedElement) {
        const r = window.getComputedStyle(this.selectedElement), p = (parseInt(r.zIndex) || 0) + 1;
        this.callbacks.onStyleChange("position", "relative", this.selectedElement), this.callbacks.onStyleChange("zIndex", String(p), this.selectedElement), this.showToast(`Brought item forward (layer depth: ${p})`, "info");
      }
    }), d && d.addEventListener("click", () => {
      if (this.callbacks.onStyleChange && this.selectedElement) {
        const r = window.getComputedStyle(this.selectedElement), p = (parseInt(r.zIndex) || 0) - 1;
        this.callbacks.onStyleChange("position", "relative", this.selectedElement), this.callbacks.onStyleChange("zIndex", String(p), this.selectedElement), this.showToast(`Sent item backward (layer depth: ${p})`, "info");
      }
    }), f && f.addEventListener("click", () => {
      this.callbacks.onStyleChange && this.selectedElement && (this.callbacks.onStyleChange("position", "relative", this.selectedElement), this.callbacks.onStyleChange("zIndex", "-1", this.selectedElement), this.showToast("Sent item to absolute back", "danger"));
    });
    const h = () => this.modalOverlay.classList.remove("open");
    this.shadowRoot.getElementById("modal-close-btn").addEventListener("click", h), this.shadowRoot.getElementById("modal-ok-btn").addEventListener("click", h), this.inspector.addEventListener("mouseenter", () => {
    ...
    }), this.shadowRoot.addEventListener("input", (r) => {
      const c = r.target;
      if (c.dataset.style) {
        const p = c.dataset.style;
        let b = c.value;
        if (c.type === "number" && b !== "" && p !== "opacity" && p !== "zIndex" && (b = `${b}px`), c.type === "color") {
          const v = this.shadowRoot.querySelector(`[data-style-sync="${p}"]`);
          v && (v.value = b), c.parentElement.style.backgroundColor = b;
        }
        this.triggerStyleChange(p, b);
      }
      if (c.dataset.styleSync) {
        const p = c.dataset.styleSync, b = c.value, v = this.shadowRoot.querySelector(`input[type="color"][data-style="${p}"]`);
        if (v) {
          const g = w(b);
          v.value = g, v.parentElement.style.backgroundColor = b;
        }
        this.triggerStyleChange(p, b);
      }
    }), this.shadowRoot.getElementById("inspector-text-content").addEventListener("input", (r) => {
      this.callbacks.onTextChange && this.callbacks.onTextChange(r.target.value);
    }), this.shadowRoot.querySelectorAll(".align-btn").forEach((r) => {
      r.addEventListener("click", (c) => {
        const p = c.currentTarget, b = p.dataset.style, v = p.dataset.val;
        p.parentElement.querySelectorAll(".align-btn").forEach((g) => g.classList.remove("active")), p.classList.add("active"), this.triggerStyleChange(b, v);
      });
    }), this.shadowRoot.querySelectorAll(".info-icon").forEach((r) => {
      const c = () => {
        const b = r.dataset.tip, v = r.dataset.example;
        let g = b;
        v && (g += `<span class="tooltip-example">Example: ${v}</span>`), this.tooltip.innerHTML = g, this.tooltip.classList.add("show");
        const m = r.getBoundingClientRect(), E = this.tooltip.getBoundingClientRect(), I = m.top - E.height - 8, F = m.left + m.width / 2 - E.width / 2, O = Math.max(10, Math.min(window.innerWidth - E.width - 10, F)), q = I < 10 ? m.bottom + 8 : I;
        this.tooltip.style.top = `${q}px`, this.tooltip.style.left = `${O}px`;
      }, p = () => {
        this.tooltip.classList.remove("show");
      };
      r.addEventListener("mouseenter", c), r.addEventListener("mouseleave", p), r.addEventListener("focus", c), r.addEventListener("blur", p);
    });
  }
  triggerStyleChange(t, e) {
    this.callbacks.onStyleChange && this.callbacks.onStyleChange(t, e);
  }
  /* --- External Controller APIs --- */
  setBadgeState(t) {
    t ? (this.badge.classList.add("active"), this.badge.querySelector(".badge-text").textContent = "Visual Active") : (this.badge.classList.remove("active"), this.badge.querySelector(".badge-text").textContent = "Visual Editor", this.closeInspector(), this.hideOverlays());
  }
  showHover(t, e) {
    t && (this.hoverOverlay.style.display = "block", this.hoverOverlay.style.top = `${t.top}px`, this.hoverOverlay.style.left = `${t.left}px`, this.hoverOverlay.style.width = `${t.width}px`, this.hoverOverlay.style.height = `${t.height}px`, this.hoverOverlay.style.borderRadius = t.borderRadius || "0px", this.hoverOverlay.querySelector(".overlay-label").textContent = e);
  }
  hideHover() {
    this.hoverOverlay.style.display = "none";
  }
  showSelection(t, e, o) {
    t && (this.selectionOverlay.style.display = "block", this.selectionOverlay.style.top = `${t.top}px`, this.selectionOverlay.style.left = `${t.left}px`, this.selectionOverlay.style.width = `${t.width}px`, this.selectionOverlay.style.height = `${t.height}px`, this.selectionOverlay.style.borderRadius = t.borderRadius || "0px", this.selectionOverlay.querySelector(".overlay-label").textContent = e, this.selectionOverlay.querySelector(".dimension-label").textContent = `${Math.round(t.originalWidth)}px × ${Math.round(t.originalHeight)}px`, this.updateQuickPopover(o));
  }
  updateSelectionRect(t, e) {
    t && (this.selectionOverlay.style.top = `${t.top}px`, this.selectionOverlay.style.left = `${t.left}px`, this.selectionOverlay.style.width = `${t.width}px`, this.selectionOverlay.style.height = `${t.height}px`, this.selectionOverlay.style.borderRadius = t.borderRadius || "0px", this.selectionOverlay.querySelector(".dimension-label").textContent = `${Math.round(t.originalWidth)}px × ${Math.round(t.originalHeight)}px`, this.updateQuickPopover(e));
  }
  hideSelection() {
    this.selectionOverlay.style.display = "none", this.updateQuickPopover(null);
  }
  hideOverlays() {
    this.hideHover(), this.hideSelection();
  }
  openInspector(t, e, o = "", a = null) {
    var h, u, r, c;
    this.selectedElement = a, this.shadowRoot.getElementById("inspector-element-title").textContent = t, this.fillInspectorValues(e);
    const s = this.shadowRoot.getElementById("inspector-text-content");
    s && (s.value = o);
    const l = "body", n = ((u = (h = window.canvasDraftChanges) == null ? void 0 : h[l]) == null ? void 0 : u.styles) || ((c = (r = window.canvasSavedModifications) == null ? void 0 : r[l]) == null ? void 0 : c.styles), d = this.shadowRoot.getElementById("canvas-global-font-input");
    d && (d.value = (n == null ? void 0 : n.fontFamily) || ""), this.inspector.classList.add("open");
    const f = this.shadowRoot.querySelector(".tab-btn.active");
    f && f.dataset.tab === "layers" && this.updateLayersTree();
  }
  closeInspector() {
    this.inspector.classList.remove("open");
  }
  fillInspectorValues(t) {
    Object.values(this.inputs).flat().forEach((e) => {
      e.type === "color" ? (e.value = "#000000", e.parentElement.style.backgroundColor = "transparent") : (e.tagName, e.value = "");
    }), this.shadowRoot.querySelectorAll("[data-style-sync]").forEach((e) => e.value = ""), this.shadowRoot.querySelectorAll(".align-btn").forEach((e) => e.classList.remove("active")), Object.entries(t).forEach(([e, o]) => {
      const a = this.inputs[e];
      a && a.forEach((s) => {
        if (s.type === "number")
          s.value = parseFloat(o) || "";
        else if (s.type === "color") {
          const l = $(o) || "#000000";
          s.value = l, s.parentElement.style.backgroundColor = o;
          const n = this.shadowRoot.querySelector(`[data-style-sync="${e}"]`);
          n && (n.value = o);
        } else s.tagName === "SELECT" ? s.value = o : s.classList.contains("align-btn") ? s.dataset.val === o && s.classList.add("active") : s.value = o;
      });
    });
  }
  showToast(t, e = "success") {
    const o = document.createElement("div");
    o.className = `canvas-toast toast-${e}`;
    let a = "⚡";
    e === "success" && (a = "✅"), e === "danger" && (a = "🚨"), e === "info" && (a = "ℹ️"), o.innerHTML = `<span>${a}</span> <span>${t}</span>`, this.toastContainer.appendChild(o), setTimeout(() => {
      o.style.animation = "none", o.style.opacity = "1", o.style.transform = "translateY(0)", setTimeout(() => {
        o.style.transition = "all 0.3s ease", o.style.opacity = "0", o.style.transform = "translateY(20px)", setTimeout(() => o.remove(), 300);
      }, 2500);
    }, 10);
  }
  updateLayersTree() {
    const t = this.shadowRoot.getElementById("layers-tree-container");
    if (!t) return;
    if (!this.selectedElement) {
      t.innerHTML = `<div style="font-size: 12px; color: var(--text-muted); text-align: center; padding: 20px 0;">
        Select an item on the screen to view its hierarchy
      </div>`;
      return;
    }
    ...
    e.className = "layers-tree";
    const o = this.selectedElement.parentElement;
    o && o !== document.body && o !== document.documentElement && e.appendChild(this.createLayerNodeElement(o, "parent")), o ? Array.from(o.children).forEach((a) => {
      a.id !== "canvas-editor-root" && (a === this.selectedElement ? e.appendChild(this.createLayerNodeElement(a, "selected")) : e.appendChild(this.createLayerNodeElement(a, "sibling")));
    }) : e.appendChild(this.createLayerNodeElement(this.selectedElement, "selected")), Array.from(this.selectedElement.children).forEach((a) => {
      a.id !== "canvas-editor-root" && e.appendChild(this.createLayerNodeElement(a, "child"));
    }), t.appendChild(e);
  }
  createLayerNodeElement(t, e) {
    const o = document.createElement("div");
    o.className = `layers-node ${e}-node ${e === "selected" ? "selected" : ""}`;
    const a = t.tagName.toLowerCase(), s = t.className ? `.${Array.from(t.classList).join(".")}` : "", l = s.length > 18 ? s.substring(0, 18) + "..." : s;
    o.addEventListener("click", (d) => {
      d.target.closest(".node-action-btn") || this.callbacks.onLayerSelect && this.callbacks.onLayerSelect(t);
    });
    let n = e;
    return e === "selected" && (n = "Active"), o.innerHTML = `
      <div class="node-info">
    ...
    `, o.querySelector(".toggle-visibility").addEventListener("click", (d) => {
      d.stopPropagation();
      const h = (t.style.display || window.getComputedStyle(t).display) === "none" ? "block" : "none";
      t.style.display = h, this.callbacks.onStyleChange && this.callbacks.onStyleChange("display", h, t), this.showToast(`Element visibility set to ${h}`, "info"), this.updateLayersTree();
    }), o.querySelector(".delete").addEventListener("click", (d) => {
      d.stopPropagation(), confirm(`Are you sure you want to delete this <${a}> element?`) && (t.style.display = "none", this.callbacks.onStyleChange && this.callbacks.onStyleChange("display", "none", t), this.showToast(`Deleted <${a}> element`, "danger"), e === "selected" && this.callbacks.onDeselect ? this.callbacks.onDeselect() : this.updateLayersTree());
    }), o;
  }
  updateQuickPopover(t) {
    const e = this.shadowRoot.getElementById("canvas-quick-popover");
    if (!e) return;
    if (!t) {
    ...
    const o = t.tagName.toLowerCase(), a = window.getComputedStyle(t);
    let s = "container";
    ["h1", "h2", "h3", "h4", "h5", "h6", "p", "span", "strong", "em", "li"].includes(o) ? s = "text" : o === "img" ? s = "image" : o === "button" || o === "a" && t.classList.contains("btn") || t.classList.contains("button") ? s = "button" : o === "a" && (s = "link");
    let l = "";
    s === "text" ? l = `
        ...
        <div class="popover-row">
          ...
          <div class="color-picker-row">
            <div class="color-preview-box" style="background-color: ${a.color};">
              <input type="color" class="color-native-input" data-style="color" value="${w(a.color)}">
            </div>
        ...
        <div class="popover-row">
          ...
          <div class="color-picker-row">
            <div class="color-preview-box" style="background-color: ${a.backgroundColor};">
              <input type="color" class="color-native-input" data-style="backgroundColor" value="${w(a.backgroundColor)}">
            </div>
      ...
      `, e.innerHTML = l;
    const n = e.querySelector(".popover-text-input");
    n && n.addEventListener("input", (h) => {
      const u = h.target.value;
      t.children.length === 0 ? t.textContent = u : t.innerHTML = u;
      const r = this.shadowRoot.getElementById("inspector-text-content");
      r && (r.value = u), this.callbacks.onTextChange && this.callbacks.onTextChange(u);
    });
    const d = e.querySelector(".popover-link-href");
    d && d.addEventListener("input", (h) => {
      const u = h.target.value;
      t.setAttribute("href", u), this.callbacks.onStyleChange && this.callbacks.onStyleChange("href", u, t);
    });
    const f = e.querySelector(".popover-image-src");
    f && f.addEventListener("input", (h) => {
      const u = h.target.value;
      t.src = u, this.callbacks.onStyleChange && this.callbacks.onStyleChange("src", u, t);
    }), this.cacheInputs();
  }
}
function $(i) {
  ...
  const t = i.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)$/);
  if (!t) return null;
  if ((t[4] !== void 0 ? parseFloat(t[4]) : 1) === 0) return "#ffffff";
  const o = parseInt(t[1]).toString(16).padStart(2, "0"), a = parseInt(t[2]).toString(16).padStart(2, "0"), s = parseInt(t[3]).toString(16).padStart(2, "0");
  return `#${o}${a}${s}`;
}
function w(i) {
  if (!i || i === "transparent") return "#ffffff";
  const t = document.createElement("div");
  t.style.color = i, document.body.appendChild(t);
  const e = window.getComputedStyle(t).color;
  return document.body.removeChild(t), $(e) || "#ffffff";
}
function C(i) {
  ...
  for (; e && e.nodeType === Node.ELEMENT_NODE; ) {
    ...
    if (e.classList && e.classList.length > 0) {
      const n = Array.from(e.classList).map((d) => CSS.escape(d)).join(".");
      n && (o += "." + n);
    }
    let a = e, s = 1;
    for (; a = a.previousElementSibling; )
      a.nodeName === e.nodeName && s++;
    let l = !1;
    for (a = e; a = a.nextElementSibling; )
      if (a.nodeName === e.nodeName) {
        l = !0;
        break;
      }
    (s > 1 || l) && (o += `:nth-of-type(${s})`), t.unshift(o), e = e.parentNode;
  }
  return t.join(" > ");
}
function P(i) {
...
const z = "canvas_saved_modifications", k = /* @__PURE__ */ new Map();
function T() {
...
function D(i) {
  const t = R();
  t[T()] = i, localStorage.setItem(z, JSON.stringify(t));
}
function B(i) {
  if (!i || (!i.startsWith("http") && !i.startsWith("//") && (i = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(i.trim())}:wght@300;400;500;600;700;900&display=swap`), document.querySelector(`link[href="${i}"]`))) return;
  const t = document.createElement("link");
  t.rel = "stylesheet", t.href = i, document.head.appendChild(t);
}
function S(i, t = !1) {
  Object.entries(i).forEach(([e, o]) => {
    ...
    if (a) {
      if (!k.has(e)) {
        const s = {};
        o.styles && Object.keys(o.styles).forEach((l) => {
          s[l] = a.style[l] || "";
        }), k.set(e, {
          styles: s,
          text: a.innerHTML,
      ...
      o.styles && Object.entries(o.styles).forEach(([s, l]) => {
        a.style[s] = l;
      }), o.text !== void 0 && o.text !== null && (a.children.length === 0 ? a.textContent = o.text : a.innerHTML = o.text);
    }
  });
}
function M(i) {
  Object.keys(i).forEach((t) => {
    ...
    const e = document.querySelector(t), o = k.get(t);
    e && o && (o.styles && Object.entries(o.styles).forEach(([a, s]) => {
      e.style[a] = s;
    }), o.text !== void 0 && (e.innerHTML = o.text)), k.delete(t);
  });
}
function _() {
  const i = y();
  i.__fontImports && Array.isArray(i.__fontImports) && i.__fontImports.forEach((t) => {
    try {
      B(t);
    } catch (e) {
      console.error("Canvas: Failed to inject font URL", t, e);
    }
  }), Q(), S(i, !1);
}
function j() {
  ...
  M(i);
  const t = R();
  delete t[T()], localStorage.setItem(z, JSON.stringify(t));
}
const W = "CanvasFontDB", Y = 1, x = "fonts";
function A() {
  return new Promise((i, t) => {
    const e = indexedDB.open(W, Y);
    e.onupgradeneeded = () => {
      const o = e.result;
      o.objectStoreNames.contains(x) || o.createObjectStore(x, { keyPath: "name" });
    }, e.onsuccess = () => i(e.result), e.onerror = () => t(e.error);
  });
}
async function G(i, t) {
  const e = await A();
  return new Promise((o, a) => {
    const s = e.transaction(x, "readwrite");
    s.objectStore(x).put({ name: i, data: t }), s.oncomplete = () => o(), s.onerror = () => a(s.error);
  });
}
async function U() {
  const i = await A();
  return new Promise((t, e) => {
    const s = i.transaction(x, "readonly").objectStore(x).getAll();
...
class X {
  constructor() {
    this.active = !1, this.selectedElement = null, this.selectedSelector = "", this._originalOverflow = "", this.draftChanges = {}, this.copiedAppearance = null, window.canvasDraftChanges = this.draftChanges, window.canvasSavedModifications = y(), this.handleMouseMove = this.handleMouseMove.bind(this), this.handleMouseDown = this.handleMouseDown.bind(this), this.handleMouseClick = this.handleMouseClick.bind(this), this.handleDragStart = this.handleDragStart.bind(this), this.handleDragMove = this.handleDragMove.bind(this), this.handleDragEnd = this.handleDragEnd.bind(this), this.handleResizeStart = this.handleResizeStart.bind(this), this.handleResizeMove = this.handleResizeMove.bind(this), this.handleResizeEnd = this.handleResizeEnd.bind(this), this.handleDoubleClick = this.handleDoubleClick.bind(this), this.handleTextChange = this.handleTextChange.bind(this), this.handleFontImport = this.handleFontImport.bind(this), this.dragState = {
      ...
      onStyleChange: (t, e, o) => this.handleStyleChange(t, e, o),
      onTextChange: (t) => this.handleTextChange(t),
      onFontImport: (t) => this.handleFontImport(t),
      onLocalFontUpload: (t) => this.handleLocalFontUpload(t),
      onQuickAction: (t, e) => this.handleQuickAction(t, e),
      onLayerSelect: (t) => this.selectElement(t),
      onDeselect: () => this.deselectElement(),
      onSave: () => this.saveChanges(),
      onCancel: () => this.cancelChanges(),
      onReset: () => this.resetAllChanges(),
      onGlobalFontChange: (t) => this.handleGlobalFontChange(t)
    }), this.setupGlobalEvents();
  }
  setupGlobalEvents() {
    window.addEventListener("keydown", (t) => {
      this.active && t.key === "Escape" && (this.selectedElement ? (this.selectedElement.contentEditable === "true" && this.selectedElement.blur(), this.deselectElement()) : this.toggleActive());
    }), window.addEventListener("resize", () => {
      this.active && this.repositionOverlays();
    });
  }
  toggleActive() {
    this.active = !this.active, this.ui.setBadgeState(this.active), this.active ? (this.ui.showToast("Canvas activated. Click elements to edit!", "info"), document.body.style.cursor = "crosshair", this.addPageListeners()) : (this.ui.showToast("Canvas deactivated.", "info"), document.body.style.cursor = "", this.deselectElement(), this.removePageListeners());
  }
  addPageListeners() {
    document.addEventListener("mousemove", this.handleMouseMove, !0), document.addEventListener("mousedown", this.handleMouseDown, !0), document.addEventListener("click", this.handleMouseClick, !0), document.addEventListener("dblclick", this.handleDoubleClick, !0), this.ui.selectionOverlay.querySelector('[data-action="drag"]').addEventListener("mousedown", this.handleDragStart), this.ui.selectionOverlay.querySelectorAll(".resize-handle").forEach((e) => {
      e.addEventListener("mousedown", this.handleResizeStart);
    });
  }
  removePageListeners() {
    document.removeEventListener("mousemove", this.handleMouseMove, !0), document.removeEventListener("mousedown", this.handleMouseDown, !0), document.removeEventListener("click", this.handleMouseClick, !0), document.removeEventListener("dblclick", this.handleDoubleClick, !0);
  }
  isEditorElement(t) {
    return t === this.ui.root || this.ui.root.contains(t);
  }
  /**
   * Applies a concentric offset padding (6px) to overlays
   * so they wrap elements without covering original borders.
   */
  calculateOverlayRect(t) {
    const e = t.getBoundingClientRect(), o = window.getComputedStyle(t), a = 6;
    let s = parseFloat(o.borderRadius) || 0;
    return {
      top: e.top - a,
      left: e.left - a,
      width: e.width + a * 2,
  ...
  handleMouseMove(t) {
    if (!this.active || this.dragState.isDragging || this.resizeState.isResizing || this.isEditorElement(t.target)) return;
    const e = t.target, o = `${e.tagName.toLowerCase()}${e.classList.length ? "." + Array.from(e.classList).join(".") : ""}`, a = this.calculateOverlayRect(e);
    this.ui.showHover(a, o);
  }
  handleMouseDown(t) {
    if (this.active && !this.isEditorElement(t.target))
      if (this.selectedElement && this.selectedElement.contains(t.target)) {
        const e = t.clientX, o = t.clientY;
        let a = !1;
        const s = (n) => {
          Math.hypot(n.clientX - e, n.clientY - o) > 4 && (a = !0, document.removeEventListener("mousemove", s, !0), document.removeEventListener("mouseup", l, !0), this.handleDragStart(t));
        }, l = (n) => {
          document.removeEventListener("mousemove", s, !0), document.removeEventListener("mouseup", l, !0), a || t.target !== this.selectedElement && this.selectElement(t.target);
        };
        document.addEventListener("mousemove", s, !0), document.addEventListener("mouseup", l, !0), t.preventDefault(), t.stopPropagation();
      } else
        t.preventDefault(), t.stopPropagation(), this.selectElement(t.target);
  }
  handleMouseClick(t) {
    this.active && (this.isEditorElement(t.target) || (t.preventDefault(), t.stopPropagation()));
  }
  selectElement(t) {
    if (this.selectedElement === t) return;
    this.deselectElement(), this.selectedElement = t, this.selectedSelector = C(t);
    const e = t.getBoundingClientRect(), o = e.left + e.width / 2, a = window.innerWidth / 2;
    o > a ? (this.ui.inspector.classList.remove("dock-right"), this.ui.inspector.classList.add("dock-left")) : (this.ui.inspector.classList.remove("dock-left"), this.ui.inspector.classList.add("dock-right")), this._originalOverflow = document.documentElement.style.overflow, document.documentElement.style.overflow = "hidden";
    const s = this.calculateOverlayRect(t), l = `${t.tagName.toLowerCase()}${t.id ? "#" + t.id : ""}`;
    this.ui.showSelection(s, l, t);
    const n = window.getComputedStyle(t), d = {
      ...
      width: t.style.width || n.width,
      height: t.style.height || n.height,
      fontSize: n.fontSize,
    ...
    }, f = t.children.length === 0 ? t.textContent.trim() : t.innerHTML.trim();
    this.ui.openInspector(l, d, f, t);
  }
  deselectElement() {
    this.selectedElement && (this.selectedElement.contentEditable === "true" && this.selectedElement.blur(), this._originalOverflow !== void 0 ? document.documentElement.style.overflow = this._originalOverflow : document.documentElement.style.overflow = "", this.selectedElement = null, this.selectedSelector = "", this.ui.selectedElement = null), this.ui.hideSelection(), this.ui.closeInspector();
  }
  repositionOverlays() {
    if (!this.selectedElement) return;
    const t = this.calculateOverlayRect(this.selectedElement);
    this.ui.updateSelectionRect(t, this.selectedElement);
  }
  /* --- Text Editing Engine --- */
  handleDoubleClick(t) {
    if (!this.active || this.isEditorElement(t.target)) return;
    t.preventDefault(), t.stopPropagation();
    const e = t.target;
    this.selectElement(e), e.contentEditable = "true", e.focus();
    const o = document.createRange();
    o.selectNodeContents(e), o.collapse(!1);
    const a = window.getSelection();
    a.removeAllRanges(), a.addRange(o), e.style.outline = "2px dashed var(--accent-color)";
    const s = () => {
      e.contentEditable = "false", e.style.outline = "";
      const n = e.innerHTML;
      this.recordTextChange(this.selectedSelector, n), this.repositionOverlays();
      const d = this.ui.shadowRoot.getElementById("inspector-text-content");
      d && (d.value = e.children.length === 0 ? e.textContent.trim() : e.innerHTML.trim()), e.removeEventListener("blur", s), e.removeEventListener("keydown", l);
    }, l = (n) => {
      var d, f;
      if (n.key === "Escape") {
        const h = (d = this.draftChanges[this.selectedSelector]) == null ? void 0 : d.text;
        if (h !== void 0)
          e.innerHTML = h;
        else {
          const u = (f = y()[this.selectedSelector]) == null ? void 0 : f.text;
          u !== void 0 && (e.innerHTML = u);
        }
        e.blur();
      } else n.key === "Enter" && !n.shiftKey && (n.preventDefault(), e.blur());
    };
    e.addEventListener("blur", s), e.addEventListener("keydown", l);
  }
  recordTextChange(t, e) {
    this.draftChanges[t] || (this.draftChanges[t] = { styles: {}, text: "" }), this.draftChanges[t].text = e, S(this.draftChanges, !0);
  }
  handleTextChange(t) {
    this.selectedElement && (this.selectedElement.children.length === 0 ? this.selectedElement.textContent = t : this.selectedElement.innerHTML = t, this.recordTextChange(this.selectedSelector, t), this.repositionOverlays());
  }
  handleFontImport(t) {
    if (t)
      try {
        ...
        if (t.includes("family=")) {
          const l = t.match(/family=([^&:]+)/);
          l && (e = decodeURIComponent(l[1].split(":")[0].replace(/\+/g, " ")));
        }
        this.handleStyleChange("fontFamily", e);
        const o = y();
        o.__fontImports || (o.__fontImports = []), o.__fontImports.includes(t) || (o.__fontImports.push(t), D(o));
        const a = this.ui.shadowRoot.querySelector('input[data-style="fontFamily"]');
        a && (a.value = e);
        const s = this.ui.shadowRoot.getElementById("canvas-font-import-url");
        s && (s.value = ""), this.ui.showToast(`Font "${e}" loaded and applied successfully!`, "success");
      } catch (e) {
        console.error("Canvas: Failed to import font", e), this.ui.showToast("Failed to load custom font.", "danger");
      }
  }
  handleQuickAction(t, e) {
    if (e)
      switch (t) {
        case "parent": {
          const o = e.parentElement;
          o && o !== document.body && o !== document.documentElement ? (this.selectElement(o), this.ui.showToast(`Selected parent: <${o.tagName.toLowerCase()}>`, "info")) : this.ui.showToast("No higher parent element select-eligible", "info");
          break;
        }
        case "child": {
          const o = e.firstElementChild;
          o && o.id !== "canvas-editor-root" ? (this.selectElement(o), this.ui.showToast(`Selected child: <${o.tagName.toLowerCase()}>`, "info")) : this.ui.showToast("No select-eligible child elements found", "info");
          break;
        }
        case "edit-text": {
          this.handleDoubleClick({ target: e, preventDefault: () => {
          } }), this.ui.showToast("Inline editing mode activated!", "info");
          break;
        }
        case "visibility": {
          const a = (e.style.display || window.getComputedStyle(e).display) === "none" ? "block" : "none";
          e.style.display = a, this.handleStyleChange("display", a, e), this.ui.showToast(`Element visibility set to ${a}`, "info"), a === "none" ? this.deselectElement() : this.repositionOverlays();
          break;
        }
        case "duplicate": {
          const o = e.cloneNode(!0);
          o.id && (o.id = `${o.id}-clone-${Date.now().toString().slice(-4)}`), e.parentNode.insertBefore(o, e.nextSibling);
          const a = C(o);
          this.draftChanges[a] = {
            styles: { display: o.style.display || "" },
            text: o.children.length === 0 ? o.textContent : o.innerHTML
          }, this.ui.showToast(`Duplicated <${e.tagName.toLowerCase()}> element`, "success"), this.selectElement(o);
          break;
        }
        case "delete": {
          confirm(`Are you sure you want to delete this <${e.tagName.toLowerCase()}> element?`) && (e.style.display = "none", this.handleStyleChange("display", "none", e), this.deselectElement(), this.ui.showToast("Deleted element", "danger"));
          break;
        }
        case "copy-style": {
          ...
          this.copiedAppearance = {
            ...
            boxShadow: e.style.boxShadow || o.boxShadow || ""
        ...
        case "paste-style": {
          ...
          Object.entries(this.copiedAppearance).forEach(([l, n]) => {
            this.handleStyleChange(l, n, e);
          });
          const o = {};
          Object.keys(this.copiedAppearance).forEach((l) => {
            o[l] = e.style[l] || window.getComputedStyle(e)[l];
          });
          const a = e.children.length === 0 ? e.textContent.trim() : e.innerHTML.trim(), s = `${e.tagName.toLowerCase()}${e.id ? "#" + e.id : ""}`;
          this.ui.openInspector(s, o, a, e), this.ui.showToast("Pasted styles onto selected item!", "success");
          break;
        }
      }
  }
  handleLocalFontUpload(t) {
    !t || t.length === 0 || t.forEach((e) => {
      const o = e.name.split(".")[0].replace(/[^a-zA-Z0-9]/g, "-"), a = new FileReader();
      a.onload = async (s) => {
        const l = s.target.result;
        try {
          const n = new FontFace(o, l);
          await n.load(), document.fonts.add(n), await G(o, l), this.handleStyleChange("fontFamily", o);
          const d = this.ui.shadowRoot.querySelector('input[data-style="fontFamily"]');
          d && (d.value = o), this.ui.showToast(`Custom font "${o}" uploaded & applied!`, "success"), this.selectedElement && this.ui.updateLayersTree();
        } catch (n) {
          console.error(`Failed to register custom font file: ${e.name}`, n), this.ui.showToast(`Failed to parse font file: ${e.name}`, "danger");
        }
  ...
  handleDragStart(t) {
    ...
    t.preventDefault();
    const e = this.selectedElement, o = window.getComputedStyle(e);
    o.position === "static" && (e.style.position = "relative", this.recordStyleChange("position", "relative")), this._originalTransition = e.style.transition || "", e.style.setProperty("transition", "none", "important"), this.dragState = {
      isDragging: !0,
      startX: t.clientX,
      startY: t.clientY,
      startLeft: parseFloat(o.left) || 0,
      startTop: parseFloat(o.top) || 0
  ...
  handleDragMove(t) {
    ...
    const e = t.clientX - this.dragState.startX, o = t.clientY - this.dragState.startY, a = this.dragState.startLeft + e, s = this.dragState.startTop + o;
    this.selectedElement.style.left = `${a}px`, this.selectedElement.style.top = `${s}px`, this.recordStyleChange("left", `${a}px`), this.recordStyleChange("top", `${s}px`), this.repositionOverlays();
  }
  handleDragEnd() {
  ...
  handleResizeStart(t) {
    if (!this.active || !this.selectedElement) return;
    t.preventDefault(), t.stopPropagation();
    const e = this.selectedElement, o = window.getComputedStyle(e), a = e.getBoundingClientRect();
    this._originalTransition = e.style.transition || "", e.style.setProperty("transition", "none", "important"), this.resizeState = {
      isResizing: !0,
      handle: t.target.dataset.handle,
      startX: t.clientX,
      startY: t.clientY,
      startWidth: a.width,
      ...
      startLeft: parseFloat(o.left) || 0,
      startTop: parseFloat(o.top) || 0
    }, document.addEventListener("mousemove", this.handleResizeMove), document.addEventListener("mouseup", this.handleResizeEnd);
  }
  handleResizeMove(t) {
    if (!this.resizeState.isResizing || !this.selectedElement) return;
    const e = this.selectedElement, o = this.resizeState, a = t.clientX - o.startX, s = t.clientY - o.startY;
    let l = o.startWidth, n = o.startHeight, d = o.startLeft, f = o.startTop;
    const h = 10;
    if (o.handle.includes("r") && (l = Math.max(h, o.startWidth + a)), o.handle.includes("b") && (n = Math.max(h, o.startHeight + s)), o.handle.includes("l")) {
      const u = o.startWidth - a;
      u > h && (l = u, d = o.startLeft + a);
    }
    if (o.handle.includes("t")) {
      const u = o.startHeight - s;
      u > h && (n = u, f = o.startTop + s);
    }
    (o.handle.includes("r") || o.handle.includes("l")) && (e.style.width = `${l}px`, this.recordStyleChange("width", `${l}px`)), (o.handle.includes("b") || o.handle.includes("t")) && (e.style.height = `${n}px`, this.recordStyleChange("height", `${n}px`)), o.handle.includes("l") && (e.style.left = `${d}px`, this.recordStyleChange("left", `${d}px`)), o.handle.includes("t") && (e.style.top = `${f}px`, this.recordStyleChange("top", `${f}px`)), this.repositionOverlays();
  }
  handleResizeEnd() {
  ...
  handleStyleChange(t, e, o = this.selectedElement) {
    o && (o.style[t] = e, this.recordStyleChange(t, e, o), o === this.selectedElement && this.repositionOverlays());
  }
  recordStyleChange(t, e, o = this.selectedElement) {
    const a = C(o);
    a && (this.draftChanges[a] || (this.draftChanges[a] = { styles: {}, text: void 0 }), this.draftChanges[a].styles || (this.draftChanges[a].styles = {}), this.draftChanges[a].styles[t] = e);
  }
  /* --- Save / Cancel Actions --- */
  saveChanges() {
    const t = y();
    Object.entries(this.draftChanges).forEach(([o, a]) => {
      t[o] || (t[o] = { styles: {}, text: void 0 }), a.styles && (t[o].styles = {
        ...t[o].styles,
        ...a.styles
      }), a.text !== void 0 && (t[o].text = a.text);
    }), D(t), this.draftChanges = {};
    const e = this.generateCSSExport(t);
    this.ui.showToast("All modifications saved successfully!", "success"), this.ui.showCodeExport(e);
  }
  cancelChanges() {
    M(this.draftChanges), this.draftChanges = {}, this.ui.showToast("Changes discarded.", "info"), this.deselectElement();
  }
  resetAllChanges() {
    j(), this.draftChanges = {}, this.ui.showToast("All visual overrides reset.", "danger"), this.deselectElement();
  }
  generateCSSExport(t) {
    let e = "";
    return Object.entries(t).forEach(([o, a]) => {
      const s = a.styles || {}, l = Object.keys(s);
      l.length > 0 && (e += `${o} {
`, l.forEach((n) => {
        const d = P(n);
        e += `  ${d}: ${s[n]};
...
`);
  ...
  handleGlobalFontChange(t) {
    t && (document.body.style.fontFamily = t, this.draftChanges.body || (this.draftChanges.body = { styles: {}, text: void 0 }), this.draftChanges.body.styles || (this.draftChanges.body.styles = {}), this.draftChanges.body.styles.fontFamily = t, S(this.draftChanges, !0), this.ui.showToast(`Set global website font to "${t}"`, "success"));
  }
}
try {
  _();
...
function L() {
  if (!window.__CanvasEditor) {
    ...
    try {
      window.__CanvasEditor = new X(), console.log("Canvas Visual Editor initialized successfully!");
    } catch (i) {
