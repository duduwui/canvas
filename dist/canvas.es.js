...
class E {
  ...
  bindEvents() {
    ...
    this.shadowRoot.getElementById("modal-close-btn").addEventListener("click", e), this.shadowRoot.getElementById("modal-ok-btn").addEventListener("click", e), this.shadowRoot.addEventListener("input", (s) => {
      const o = s.target;
      if (o.dataset.style) {
        const a = o.dataset.style;
        let n = o.value;
        if (o.type === "number" && n !== "" && a !== "opacity" && (n = `${n}px`), o.type === "color") {
          const l = this.shadowRoot.querySelector(`[data-style-sync="${a}"]`);
          l && (l.value = n), o.parentElement.style.backgroundColor = n;
        }
        this.triggerStyleChange(a, n);
      }
      if (o.dataset.styleSync) {
        const a = o.dataset.styleSync, n = o.value, l = this.shadowRoot.querySelector(`input[type="color"][data-style="${a}"]`);
        if (l) {
          const r = C(n);
          l.value = r, l.parentElement.style.backgroundColor = n;
        }
        this.triggerStyleChange(a, n);
      }
      ...
      s.addEventListener("click", (o) => {
        const a = o.currentTarget, n = a.dataset.style, l = a.dataset.val;
        a.parentElement.querySelectorAll(".align-btn").forEach((r) => r.classList.remove("active")), a.classList.add("active"), this.triggerStyleChange(n, l);
      });
    });
  }
  ...
  openInspector(e, t, s = "") {
    this.shadowRoot.getElementById("inspector-element-title").textContent = e, this.fillInspectorValues(t);
    const o = this.shadowRoot.getElementById("inspector-text-content");
    o && (o.value = s), this.inspector.classList.add("open");
  }
  closeInspector() {
  ...
  fillInspectorValues(e) {
    Object.values(this.inputs).flat().forEach((t) => {
      ...
      if (o && (o.forEach((a) => {
        if (a.type === "number")
          a.value = parseFloat(s) || "";
        else if (a.type === "color") {
          const n = y(s) || "#000000";
          a.value = n, a.parentElement.style.backgroundColor = s;
          const l = this.shadowRoot.querySelector(`[data-style-sync="${t}"]`);
          l && (l.value = s);
        } else a.tagName === "SELECT" ? a.value = s : a.classList.contains("align-btn") ? a.dataset.val === s && a.classList.add("active") : a.value = s;
      }), t === "textAlign")) {
        const a = this.shadowRoot.querySelector(`.align-btn[data-val="${s}"]`);
...
function y(i) {
  ...
  const s = parseInt(e[1]).toString(16).padStart(2, "0"), o = parseInt(e[2]).toString(16).padStart(2, "0"), a = parseInt(e[3]).toString(16).padStart(2, "0");
  return `#${s}${o}${a}`;
}
function C(i) {
...
function S(i) {
  ...
  for (; t && t.nodeType === Node.ELEMENT_NODE; ) {
    ...
    for (; o = o.previousElementSibling; )
      o.nodeName === t.nodeName && a++;
    let n = !1;
    for (o = t; o = o.nextElementSibling; )
      if (o.nodeName === t.nodeName) {
        n = !0;
        break;
      }
    (a > 1 || n) && (s += `:nth-of-type(${a})`), e.unshift(s), t = t.parentNode;
  }
  return e.join(" > ");
}
function k(i) {
...
function x(i, e = !1) {
  Object.entries(i).forEach(([t, s]) => {
    ...
    if (o) {
      ...
      s.styles && Object.entries(s.styles).forEach(([a, n]) => {
        o.style[a] = n;
      }), s.text !== void 0 && s.text !== null && (o.children.length === 0 ? o.textContent = s.text : o.innerHTML = s.text);
    }
  });
}
function m(i) {
  Object.keys(i).forEach((e) => {
    ...
    t && s && (s.styles && Object.entries(s.styles).forEach(([o, a]) => {
      t.style[o] = a;
...
class T {
  ...
  handleMouseMove(e) {
    ...
    const t = e.target, s = t.getBoundingClientRect(), o = `${t.tagName.toLowerCase()}${t.classList.length ? "." + Array.from(t.classList).join(".") : ""}`, a = {
      top: s.top,
      left: s.left,
      width: s.width,
      height: s.height
  ...
  selectElement(e) {
    ...
    const o = window.getComputedStyle(e), a = {
      ...
      width: e.style.width || o.width,
      height: e.style.height || o.height,
      fontSize: o.fontSize,
  ...
  handleDoubleClick(e) {
    ...
    o.removeAllRanges(), o.addRange(s), t.style.outline = "2px dashed var(--accent-color)";
    const a = () => {
      t.contentEditable = "false", t.style.outline = "";
      const l = t.innerHTML;
      this.recordTextChange(this.selectedSelector, l), this.repositionOverlays(), t.removeEventListener("blur", a), t.removeEventListener("keydown", n);
    }, n = (l) => {
      var r, h;
      if (l.key === "Escape") {
  ...
  recordTextChange(e, t) {
    this.draftChanges[e] || (this.draftChanges[e] = { styles: {}, text: "" }), this.draftChanges[e].text = t, x(this.draftChanges, !0);
  }
  handleTextChange(e) {
  ...
  handleDragMove(e) {
    ...
    const t = e.clientX - this.dragState.startX, s = e.clientY - this.dragState.startY, o = this.dragState.startLeft + t, a = this.dragState.startTop + s;
    this.selectedElement.style.left = `${o}px`, this.selectedElement.style.top = `${a}px`, this.recordStyleChange("left", `${o}px`), this.recordStyleChange("top", `${a}px`), this.repositionOverlays();
  }
  handleDragEnd() {
  ...
  handleResizeStart(e) {
    if (!this.active || !this.selectedElement) return;
    e.preventDefault(), e.stopPropagation();
    const t = this.selectedElement, s = window.getComputedStyle(t), o = t.getBoundingClientRect();
    this.resizeState = {
      isResizing: !0,
  ...
  handleResizeMove(e) {
    ...
    const t = this.selectedElement, s = this.resizeState, o = e.clientX - s.startX, a = e.clientY - s.startY;
    let n = s.startWidth, l = s.startHeight, r = s.startLeft, h = s.startTop;
    const c = 10;
    if (s.handle.includes("r") && (n = Math.max(c, s.startWidth + o)), s.handle.includes("b") && (l = Math.max(c, s.startHeight + a)), s.handle.includes("l")) {
      const d = s.startWidth - o;
      d > c && (n = d, r = s.startLeft + o);
    }
    if (s.handle.includes("t")) {
      const d = s.startHeight - a;
      d > c && (l = d, h = s.startTop + a);
    }
  ...
  saveChanges() {
    const e = v();
    Object.entries(this.draftChanges).forEach(([s, o]) => {
      e[s] || (e[s] = { styles: {}, text: void 0 }), o.styles && (e[s].styles = {
        ...e[s].styles,
        ...o.styles
      }), o.text !== void 0 && (e[s].text = o.text);
    }), L(e), this.draftChanges = {};
  ...
  resetAllChanges() {
    R(), this.draftChanges = {}, this.ui.showToast("All visual overrides reset.", "danger"), this.deselectElement();
  }
  generateCSSExport(e) {
    let t = "";
    return Object.entries(e).forEach(([s, o]) => {
      const a = o.styles || {}, n = Object.keys(a);
      n.length > 0 && (t += `${s} {
`, n.forEach((l) => {
        const r = k(l);
        t += `  ${r}: ${a[l]};
...
function g() {
  if (!window.__CanvasEditor) {
    ...
    try {
      window.__CanvasEditor = new T(), console.log("Canvas Visual Editor initialized successfully!");
    } catch (i) {
