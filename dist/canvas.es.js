...
class $ {
  ...
  init() {
    ...
    t.textContent = O, this.shadowRoot.appendChild(t), this.uiContainer = document.createElement("div"), this.uiContainer.innerHTML = this.getHTMLTemplate(), this.shadowRoot.appendChild(this.uiContainer), this.badge = this.shadowRoot.querySelector(".canvas-badge"), this.hoverOverlay = this.shadowRoot.querySelector(".canvas-hover-overlay"), this.selectionOverlay = this.shadowRoot.querySelector(".canvas-selection-overlay"), this.inspector = this.shadowRoot.querySelector(".canvas-inspector"), this.modalOverlay = this.shadowRoot.querySelector(".canvas-modal-overlay"), this.toastContainer = this.shadowRoot.querySelector(".canvas-toast-container");
  ...
  bindEvents() {
    ...
    }), this.shadowRoot.querySelectorAll(".section-header").forEach((i) => {
      i.addEventListener("click", () => {
        i.parentElement.classList.toggle("collapsed");
      });
    ...
    }), this.shadowRoot.querySelectorAll(".tab-btn").forEach((i) => {
      i.addEventListener("click", () => {
        this.shadowRoot.querySelectorAll(".tab-btn").forEach((d) => d.classList.remove("active")), this.shadowRoot.querySelectorAll(".tab-content").forEach((d) => d.classList.remove("active")), i.classList.add("active");
        const l = `tab-${i.dataset.tab}`, c = this.shadowRoot.getElementById(l);
        c && c.classList.add("active"), i.dataset.tab === "layers" && this.updateLayersTree();
      });
    }), this.shadowRoot.querySelectorAll(".swatch-btn").forEach((i) => {
      i.addEventListener("click", () => {
        const l = i.closest(".control-row");
        if (l) {
          const c = i.dataset.color, d = l.querySelector(".color-text-input"), p = l.querySelector(".color-native-input");
          d && (d.value = c, d.dispatchEvent(new Event("input", { bubbles: !0 }))), p && c !== "transparent" && (p.value = c);
        }
      });
    });
    ...
    e && e.addEventListener("change", (i) => {
      ...
      o.textContent = l.map((c) => c.name).join(", "), this.callbacks.onLocalFontUpload && this.callbacks.onLocalFontUpload(l);
    });
    const a = this.shadowRoot.querySelector(".canvas-quick-toolbar");
    a && a.addEventListener("click", (i) => {
      const l = i.target.closest(".quick-btn");
      if (!l) return;
      const c = l.dataset.action;
      this.callbacks.onQuickAction && this.selectedElement && this.callbacks.onQuickAction(c, this.selectedElement);
    });
    const s = () => this.modalOverlay.classList.remove("open");
    this.shadowRoot.getElementById("modal-close-btn").addEventListener("click", s), this.shadowRoot.getElementById("modal-ok-btn").addEventListener("click", s), this.inspector.addEventListener("mouseenter", () => {
    ...
    }), this.shadowRoot.addEventListener("input", (i) => {
      const l = i.target;
      if (l.dataset.style) {
        const c = l.dataset.style;
        let d = l.value;
        if (l.type === "number" && d !== "" && c !== "opacity" && c !== "zIndex" && (d = `${d}px`), l.type === "color") {
          const p = this.shadowRoot.querySelector(`[data-style-sync="${c}"]`);
          p && (p.value = d), l.parentElement.style.backgroundColor = d;
        }
        this.triggerStyleChange(c, d);
      }
      if (l.dataset.styleSync) {
        const c = l.dataset.styleSync, d = l.value, p = this.shadowRoot.querySelector(`input[type="color"][data-style="${c}"]`);
        if (p) {
          const h = A(d);
          p.value = h, p.parentElement.style.backgroundColor = d;
        }
        this.triggerStyleChange(c, d);
      }
    }), this.shadowRoot.getElementById("inspector-text-content").addEventListener("input", (i) => {
      this.callbacks.onTextChange && this.callbacks.onTextChange(i.target.value);
    }), this.shadowRoot.querySelectorAll(".align-btn").forEach((i) => {
      i.addEventListener("click", (l) => {
        const c = l.currentTarget, d = c.dataset.style, p = c.dataset.val;
        c.parentElement.querySelectorAll(".align-btn").forEach((h) => h.classList.remove("active")), c.classList.add("active"), this.triggerStyleChange(d, p);
      });
    }), this.shadowRoot.querySelectorAll(".info-icon").forEach((i) => {
      const l = () => {
        const d = i.dataset.tip, p = i.dataset.example;
        let h = d;
        p && (h += `<span class="tooltip-example">Example: ${p}</span>`), this.tooltip.innerHTML = h, this.tooltip.classList.add("show");
        const b = i.getBoundingClientRect(), g = this.tooltip.getBoundingClientRect(), E = b.top - g.height - 8, I = b.left + b.width / 2 - g.width / 2, D = Math.max(10, Math.min(window.innerWidth - g.width - 10, I)), M = E < 10 ? b.bottom + 8 : E;
        this.tooltip.style.top = `${M}px`, this.tooltip.style.left = `${D}px`;
      }, c = () => {
        this.tooltip.classList.remove("show");
      };
      i.addEventListener("mouseenter", l), i.addEventListener("mouseleave", c), i.addEventListener("focus", l), i.addEventListener("blur", c);
    });
  }
  ...
  setBadgeState(t) {
    t ? (this.badge.classList.add("active"), this.badge.querySelector(".badge-text").textContent = "Canvas Active") : (this.badge.classList.remove("active"), this.badge.querySelector(".badge-text").textContent = "Canvas Mode", this.closeInspector(), this.hideOverlays());
  }
  showHover(t, e) {
  ...
  openInspector(t, e, o = "", a = null) {
    ...
    s && (s.value = o), this.inspector.classList.add("open");
    const r = this.shadowRoot.querySelector(".tab-btn.active");
    r && r.dataset.tab === "layers" && this.updateLayersTree();
  }
  closeInspector() {
    ...
    Object.values(this.inputs).flat().forEach((e) => {
      ...
      if (a && (a.forEach((s) => {
        if (s.type === "number")
          ...
          const r = S(o) || "#000000";
          s.value = r, s.parentElement.style.backgroundColor = o;
          const i = this.shadowRoot.querySelector(`[data-style-sync="${e}"]`);
          i && (i.value = o);
        } else s.tagName === "SELECT" ? s.value = o : s.classList.contains("align-btn") ? s.dataset.val === o && s.classList.add("active") : s.value = o;
      }), e === "textAlign")) {
        const s = this.shadowRoot.querySelector(`.align-btn[data-val="${o}"]`);
        s && s.classList.add("active");
      }
    });
  }
  showCodeExport(t) {
    const e = this.shadowRoot.getElementById("modal-css-code");
    e.textContent = t, this.modalOverlay.classList.add("open");
  }
  showToast(t, e = "info") {
    const o = document.createElement("div");
    o.className = `canvas-toast toast-${e}`, o.textContent = t, this.toastContainer.appendChild(o), setTimeout(() => {
  ...
  createLayerNodeElement(t, e) {
    ...
    const a = t.tagName.toLowerCase(), s = t.className ? `.${Array.from(t.classList).join(".")}` : "", r = s.length > 18 ? s.substring(0, 18) + "..." : s;
    o.addEventListener("click", (l) => {
      l.target.closest(".node-action-btn") || this.callbacks.onLayerSelect && this.callbacks.onLayerSelect(t);
    });
    let i = e;
    return e === "selected" && (i = "Active"), o.innerHTML = `
      <div class="node-info">
    ...
    `, o.querySelector(".toggle-visibility").addEventListener("click", (l) => {
      l.stopPropagation();
      const d = (t.style.display || window.getComputedStyle(t).display) === "none" ? "block" : "none";
      t.style.display = d, this.callbacks.onStyleChange && this.callbacks.onStyleChange("display", d, t), this.showToast(`Element visibility set to ${d}`, "info"), this.updateLayersTree();
    }), o.querySelector(".delete").addEventListener("click", (l) => {
      l.stopPropagation(), confirm(`Are you sure you want to delete this <${a}> element?`) && (t.style.display = "none", this.callbacks.onStyleChange && this.callbacks.onStyleChange("display", "none", t), this.showToast(`Deleted <${a}> element`, "danger"), e === "selected" && this.callbacks.onDeselect ? this.callbacks.onDeselect() : this.updateLayersTree());
    }), o;
  }
}
function S(n) {
  ...
  if (n === "transparent") return "#ffffff";
  const t = n.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)$/);
  if (!t) return null;
...
function A(n) {
  if (!n || n === "transparent") return "#ffffff";
  const t = document.createElement("div");
  t.style.color = n, document.body.appendChild(t);
  const e = window.getComputedStyle(t).color;
  return document.body.removeChild(t), S(e) || "#ffffff";
}
function y(n) {
  ...
  for (; e && e.nodeType === Node.ELEMENT_NODE; ) {
    ...
    if (e.classList && e.classList.length > 0) {
      const i = Array.from(e.classList).map((l) => CSS.escape(l)).join(".");
      i && (o += "." + i);
    }
    let a = e, s = 1;
    ...
    (s > 1 || r) && (o += `:nth-of-type(${s})`), t.unshift(o), e = e.parentNode;
  }
  return t.join(" > ");
}
function F(n) {
...
const m = "canvas_saved_modifications", v = /* @__PURE__ */ new Map();
function w() {
...
function k() {
  try {
    ...
    return console.error("Canvas: Failed to parse saved modifications", n), {};
  }
}
function u() {
  return k()[w()] || {};
...
function C(n) {
  const t = k();
  t[w()] = n, localStorage.setItem(m, JSON.stringify(t));
}
function L(n) {
  if (!n || (!n.startsWith("http") && !n.startsWith("//") && (n = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(n.trim())}:wght@300;400;500;600;700;900&display=swap`), document.querySelector(`link[href="${n}"]`))) return;
...
function z(n, t = !1) {
  Object.entries(n).forEach(([e, o]) => {
    ...
    if (a) {
      if (!v.has(e)) {
        const s = {};
        o.styles && Object.keys(o.styles).forEach((r) => {
          s[r] = a.style[r] || "";
        }), v.set(e, {
          styles: s,
          text: a.innerHTML,
      ...
      o.styles && Object.entries(o.styles).forEach(([s, r]) => {
        a.style[s] = r;
...
function R(n) {
  Object.keys(n).forEach((t) => {
    ...
    const e = document.querySelector(t), o = v.get(t);
    e && o && (o.styles && Object.entries(o.styles).forEach(([a, s]) => {
      e.style[a] = s;
    }), o.text !== void 0 && (e.innerHTML = o.text)), v.delete(t);
  });
}
function B() {
  const n = u();
  n.__fontImports && Array.isArray(n.__fontImports) && n.__fontImports.forEach((t) => {
    try {
      L(t);
    } catch (e) {
...
function q() {
  ...
  R(n);
  const t = k();
  delete t[w()], localStorage.setItem(m, JSON.stringify(t));
}
const N = "CanvasFontDB", H = 1, f = "fonts";
function T() {
  return new Promise((n, t) => {
    const e = indexedDB.open(N, H);
    e.onupgradeneeded = () => {
      const o = e.result;
      o.objectStoreNames.contains(f) || o.createObjectStore(f, { keyPath: "name" });
...
async function _(n, t) {
  ...
  return new Promise((o, a) => {
    const s = e.transaction(f, "readwrite");
    s.objectStore(f).put({ name: n, data: t }), s.oncomplete = () => o(), s.onerror = () => a(s.error);
  });
}
async function j() {
  const n = await T();
  return new Promise((t, e) => {
    const s = n.transaction(f, "readonly").objectStore(f).getAll();
...
async function W() {
  try {
    ...
    console.warn("Canvas: Could not access local Font DB (IndexedDB)", n);
  }
}
class P {
  constructor() {
  ...
  addPageListeners() {
    document.addEventListener("mousemove", this.handleMouseMove, !0), document.addEventListener("click", this.handleMouseClick, !0), document.addEventListener("dblclick", this.handleDoubleClick, !0), this.ui.selectionOverlay.querySelector('[data-action="drag"]').addEventListener("mousedown", this.handleDragStart), this.ui.selectionOverlay.querySelectorAll(".resize-handle").forEach((e) => {
      e.addEventListener("mousedown", this.handleResizeStart);
    });
  }
  removePageListeners() {
    document.removeEventListener("mousemove", this.handleMouseMove, !0), document.removeEventListener("click", this.handleMouseClick, !0), document.removeEventListener("dblclick", this.handleDoubleClick, !0);
  }
  isEditorElement(t) {
  ...
  handleMouseClick(t) {
    this.active && (this.isEditorElement(t.target) || (t.preventDefault(), t.stopPropagation(), this.selectElement(t.target)));
  }
  selectElement(t) {
    if (this.selectedElement === t) return;
    this.deselectElement(), this.selectedElement = t, this.selectedSelector = y(t), this._originalOverflow = document.documentElement.style.overflow, document.documentElement.style.overflow = "hidden";
    const e = this.calculateOverlayRect(t), o = `${t.tagName.toLowerCase()}${t.id ? "#" + t.id : ""}`;
    this.ui.showSelection(e, o);
    const a = window.getComputedStyle(t), s = {
      ...
      width: t.style.width || a.width,
      height: t.style.height || a.height,
      fontSize: a.fontSize,
    ...
    }, r = t.children.length === 0 ? t.textContent.trim() : t.innerHTML.trim();
    this.ui.openInspector(o, s, r, t);
  }
  deselectElement() {
  ...
  handleDoubleClick(t) {
    ...
    const s = () => {
      ...
      const i = e.innerHTML;
      this.recordTextChange(this.selectedSelector, i), this.repositionOverlays();
      const l = this.ui.shadowRoot.getElementById("inspector-text-content");
      l && (l.value = e.children.length === 0 ? e.textContent.trim() : e.innerHTML.trim()), e.removeEventListener("blur", s), e.removeEventListener("keydown", r);
    }, r = (i) => {
      var l, c;
      if (i.key === "Escape") {
        const d = (l = this.draftChanges[this.selectedSelector]) == null ? void 0 : l.text;
        if (d !== void 0)
          e.innerHTML = d;
        else {
          const p = (c = u()[this.selectedSelector]) == null ? void 0 : c.text;
          p !== void 0 && (e.innerHTML = p);
        }
        e.blur();
      } else i.key === "Enter" && !i.shiftKey && (i.preventDefault(), e.blur());
    };
    e.addEventListener("blur", s), e.addEventListener("keydown", r);
  }
  recordTextChange(t, e) {
    this.draftChanges[t] || (this.draftChanges[t] = { styles: {}, text: "" }), this.draftChanges[t].text = e, z(this.draftChanges, !0);
  }
  handleTextChange(t) {
    ...
    if (t)
      try {
        ...
        if (t.includes("family=")) {
          const r = t.match(/family=([^&:]+)/);
          r && (e = decodeURIComponent(r[1].split(":")[0].replace(/\+/g, " ")));
        }
        this.handleStyleChange("fontFamily", e);
        const o = u();
        o.__fontImports || (o.__fontImports = []), o.__fontImports.includes(t) || (o.__fontImports.push(t), C(o));
  ...
  handleLocalFontUpload(t) {
    !t || t.length === 0 || t.forEach((e) => {
      ...
      a.onload = async (s) => {
        ...
        try {
          const i = new FontFace(o, r);
          await i.load(), document.fonts.add(i), await _(o, r), this.handleStyleChange("fontFamily", o);
          const l = this.ui.shadowRoot.querySelector('input[data-style="fontFamily"]');
          l && (l.value = o), this.ui.showToast(`Custom font "${o}" uploaded & applied!`, "success"), this.selectedElement && this.ui.updateLayersTree();
        } catch (i) {
          console.error(`Failed to register custom font file: ${e.name}`, i), this.ui.showToast(`Failed to parse font file: ${e.name}`, "danger");
        }
  ...
  handleResizeMove(t) {
    ...
    let r = o.startWidth, i = o.startHeight, l = o.startLeft, c = o.startTop;
    const d = 10;
    if (o.handle.includes("r") && (r = Math.max(d, o.startWidth + a)), o.handle.includes("b") && (i = Math.max(d, o.startHeight + s)), o.handle.includes("l")) {
      const p = o.startWidth - a;
      p > d && (r = p, l = o.startLeft + a);
    }
    if (o.handle.includes("t")) {
      const p = o.startHeight - s;
      p > d && (i = p, c = o.startTop + s);
    }
    (o.handle.includes("r") || o.handle.includes("l")) && (e.style.width = `${r}px`, this.recordStyleChange("width", `${r}px`)), (o.handle.includes("b") || o.handle.includes("t")) && (e.style.height = `${i}px`, this.recordStyleChange("height", `${i}px`)), o.handle.includes("l") && (e.style.left = `${l}px`, this.recordStyleChange("left", `${l}px`)), o.handle.includes("t") && (e.style.top = `${c}px`, this.recordStyleChange("top", `${c}px`)), this.repositionOverlays();
  }
  handleResizeEnd() {
  ...
  cancelChanges() {
    R(this.draftChanges), this.draftChanges = {}, this.ui.showToast("Changes discarded.", "info"), this.deselectElement();
  }
  resetAllChanges() {
    q(), this.draftChanges = {}, this.ui.showToast("All visual overrides reset.", "danger"), this.deselectElement();
  }
  generateCSSExport(t) {
    let e = "";
    return Object.entries(t).forEach(([o, a]) => {
      const s = a.styles || {}, r = Object.keys(s);
      r.length > 0 && (e += `${o} {
`, r.forEach((i) => {
        const l = F(i);
        e += `  ${l}: ${s[i]};
...
try {
  ...
  console.error("Canvas: Failed to initialize saved modifications", n);
}
function x() {
  if (!window.__CanvasEditor) {
    ...
    try {
      window.__CanvasEditor = new P(), console.log("Canvas Visual Editor initialized successfully!");
    } catch (n) {
      console.error("Canvas: Failed to initialize editor", n);
    }
  }
}
