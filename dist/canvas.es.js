...
class w {
  ...
  cacheInputs() {
    this.shadowRoot.querySelectorAll("[data-style]").forEach((e) => {
      const t = e.dataset.style;
      this.inputs[t] || (this.inputs[t] = []), this.inputs[t].push(e);
    });
  }
  bindEvents() {
    ...
    const e = () => this.modalOverlay.classList.remove("open");
    this.shadowRoot.getElementById("modal-close-btn").addEventListener("click", e), this.shadowRoot.getElementById("modal-ok-btn").addEventListener("click", e), this.shadowRoot.addEventListener("input", (t) => {
      const s = t.target;
      if (s.dataset.style) {
        const a = s.dataset.style;
        let o = s.value;
        if (s.type === "number" && o !== "" && a !== "opacity" && (o = `${o}px`), s.type === "color") {
          const n = this.shadowRoot.querySelector(`[data-style-sync="${a}"]`);
          n && (n.value = o), s.parentElement.style.backgroundColor = o;
        }
        this.triggerStyleChange(a, o);
      }
      if (s.dataset.styleSync) {
        const a = s.dataset.styleSync, o = s.value, n = this.shadowRoot.querySelector(`input[type="color"][data-style="${a}"]`);
        n && /^#[0-9A-F]{6}$/i.test(o) && (n.value = o, n.parentElement.style.backgroundColor = o), this.triggerStyleChange(a, o);
      }
    }), this.shadowRoot.querySelectorAll(".align-btn").forEach((t) => {
      t.addEventListener("click", (s) => {
        const a = s.currentTarget, o = a.dataset.style, n = a.dataset.val;
        a.parentElement.querySelectorAll(".align-btn").forEach((l) => l.classList.remove("active")), a.classList.add("active"), this.triggerStyleChange(o, n);
      });
    });
  }
  triggerStyleChange(e, t) {
    this.callbacks.onStyleChange && this.callbacks.onStyleChange(e, t);
  }
  /* --- External Controller APIs --- */
  setBadgeState(e) {
    e ? (this.badge.classList.add("active"), this.badge.querySelector(".badge-text").textContent = "Canvas Active") : (this.badge.classList.remove("active"), this.badge.querySelector(".badge-text").textContent = "Canvas Mode", this.closeInspector(), this.hideOverlays());
  }
  showHover(e, t) {
    e && (this.hoverOverlay.style.display = "block", this.hoverOverlay.style.top = `${e.top}px`, this.hoverOverlay.style.left = `${e.left}px`, this.hoverOverlay.style.width = `${e.width}px`, this.hoverOverlay.style.height = `${e.height}px`, this.hoverOverlay.querySelector(".overlay-label").textContent = t);
  }
  hideHover() {
    this.hoverOverlay.style.display = "none";
  }
  showSelection(e, t) {
    e && (this.selectionOverlay.style.display = "block", this.selectionOverlay.style.top = `${e.top}px`, this.selectionOverlay.style.left = `${e.left}px`, this.selectionOverlay.style.width = `${e.width}px`, this.selectionOverlay.style.height = `${e.height}px`, this.selectionOverlay.querySelector(".overlay-label").textContent = t, this.selectionOverlay.querySelector(".dimension-label").textContent = `${Math.round(e.width)}px × ${Math.round(e.height)}px`);
  }
  updateSelectionRect(e) {
    e && (this.selectionOverlay.style.top = `${e.top}px`, this.selectionOverlay.style.left = `${e.left}px`, this.selectionOverlay.style.width = `${e.width}px`, this.selectionOverlay.style.height = `${e.height}px`, this.selectionOverlay.querySelector(".dimension-label").textContent = `${Math.round(e.width)}px × ${Math.round(e.height)}px`);
  }
  hideSelection() {
  ...
  openInspector(e, t) {
    this.shadowRoot.getElementById("inspector-element-title").textContent = e, this.fillInspectorValues(t), this.inspector.classList.add("open");
  }
  closeInspector() {
    this.inspector.classList.remove("open");
  }
  fillInspectorValues(e) {
    Object.values(this.inputs).flat().forEach((t) => {
      t.type === "color" ? (t.value = "#000000", t.parentElement.style.backgroundColor = "transparent") : (t.tagName, t.value = "");
    }), this.shadowRoot.querySelectorAll("[data-style-sync]").forEach((t) => t.value = ""), this.shadowRoot.querySelectorAll(".align-btn").forEach((t) => t.classList.remove("active")), Object.entries(e).forEach(([t, s]) => {
      const a = this.inputs[t];
      if (a && (a.forEach((o) => {
        if (o.type === "number")
          o.value = parseFloat(s) || "";
        else if (o.type === "color") {
          const n = E(s) || "#000000";
          o.value = n, o.parentElement.style.backgroundColor = n;
          const l = this.shadowRoot.querySelector(`[data-style-sync="${t}"]`);
          l && (l.value = s);
        } else o.tagName === "SELECT" ? o.value = s : o.classList.contains("align-btn") ? o.dataset.val === s && o.classList.add("active") : o.value = s;
      }), t === "textAlign")) {
        const o = this.shadowRoot.querySelector(`.align-btn[data-val="${s}"]`);
        o && o.classList.add("active");
      }
    });
  }
  showCodeExport(e) {
    const t = this.shadowRoot.getElementById("modal-css-code");
    t.textContent = e, this.modalOverlay.classList.add("open");
  }
  showToast(e, t = "info") {
    const s = document.createElement("div");
    s.className = `canvas-toast toast-${t}`, s.textContent = e, this.toastContainer.appendChild(s), setTimeout(() => {
      s.style.animation = "none", s.style.opacity = "1", s.style.transform = "translateY(0)", setTimeout(() => {
        s.style.transition = "all 0.3s ease", s.style.opacity = "0", s.style.transform = "translateY(20px)", setTimeout(() => s.remove(), 300);
      }, 2500);
    }, 10);
  }
}
function E(i) {
  ...
  if (!e) return null;
  const t = parseInt(e[1]).toString(16).padStart(2, "0"), s = parseInt(e[2]).toString(16).padStart(2, "0"), a = parseInt(e[3]).toString(16).padStart(2, "0");
  return `#${t}${s}${a}`;
}
function S(i) {
  ...
  for (; t && t.nodeType === Node.ELEMENT_NODE; ) {
    ...
    for (; a = a.previousElementSibling; )
      a.nodeName === t.nodeName && o++;
    let n = !1;
    for (a = t; a = a.nextElementSibling; )
      if (a.nodeName === t.nodeName) {
        n = !0;
        break;
      }
    (o > 1 || n) && (s += `:nth-of-type(${o})`), e.unshift(s), t = t.parentNode;
  }
  return e.join(" > ");
}
function C(i) {
...
const u = "canvas_saved_modifications", p = /* @__PURE__ */ new Map();
function f() {
...
function k(i) {
  const e = b();
  e[f()] = i, localStorage.setItem(u, JSON.stringify(e));
}
function y(i, e = !1) {
  Object.entries(i).forEach(([t, s]) => {
    ...
    if (a) {
      if (!p.has(t)) {
        const o = {};
        s.styles && Object.keys(s.styles).forEach((n) => {
          o[n] = a.style[n] || "";
        }), p.set(t, {
          styles: o,
          text: a.innerHTML,
      ...
      s.styles && Object.entries(s.styles).forEach(([o, n]) => {
        a.style[o] = n;
      }), s.text !== void 0 && s.text !== null && (a.children.length === 0 ? a.textContent = s.text : a.innerHTML = s.text);
    }
  });
}
function x(i) {
  Object.keys(i).forEach((e) => {
    const t = document.querySelector(e), s = p.get(e);
    t && s && (s.styles && Object.entries(s.styles).forEach(([a, o]) => {
      t.style[a] = o;
    }), s.text !== void 0 && (t.innerHTML = s.text)), p.delete(e);
  });
}
function L() {
  const i = v();
...
class R {
  ...
  setupGlobalEvents() {
    window.addEventListener("keydown", (e) => {
      this.active && e.key === "Escape" && (this.selectedElement ? (this.selectedElement.contentEditable === "true" && this.selectedElement.blur(), this.deselectElement()) : this.toggleActive());
   ...
   */
  isEditorElement(e) {
    return e === this.ui.root || this.ui.root.contains(e);
  }
  handleMouseMove(e) {
    if (!this.active || this.dragState.isDragging || this.resizeState.isResizing || this.isEditorElement(e.target)) return;
    const t = e.target, s = t.getBoundingClientRect(), a = `${t.tagName.toLowerCase()}${t.classList.length ? "." + Array.from(t.classList).join(".") : ""}`, o = {
      top: s.top,
      left: s.left,
      width: s.width,
      height: s.height
    };
    this.ui.showHover(o, a);
  }
  handleMouseClick(e) {
    this.active && (this.isEditorElement(e.target) || (e.preventDefault(), e.stopPropagation(), this.selectElement(e.target)));
  }
  selectElement(e) {
    if (this.selectedElement === e) return;
    this.deselectElement(), this.selectedElement = e, this.selectedSelector = S(e);
    const t = e.getBoundingClientRect(), s = `${e.tagName.toLowerCase()}${e.id ? "#" + e.id : ""}`;
    this.ui.showSelection(t, s);
    const a = window.getComputedStyle(e), o = {
      ...
      width: e.style.width || a.width,
      height: e.style.height || a.height,
      fontSize: a.fontSize,
    ...
    this.ui.openInspector(s, o);
  }
  deselectElement() {
    this.selectedElement && (this.selectedElement.contentEditable === "true" && this.selectedElement.blur(), this.selectedElement = null, this.selectedSelector = ""), this.ui.hideSelection(), this.ui.closeInspector();
  }
  repositionOverlays() {
    if (!this.selectedElement) return;
    const e = this.selectedElement.getBoundingClientRect();
    this.ui.updateSelectionRect(e);
  }
  /* --- Text Editing Engine --- */
  handleDoubleClick(e) {
    if (!this.active || this.isEditorElement(e.target)) return;
    e.preventDefault(), e.stopPropagation();
    const t = e.target;
    this.selectElement(t), t.contentEditable = "true", t.focus();
    const s = document.createRange();
    s.selectNodeContents(t), s.collapse(!1);
    const a = window.getSelection();
    a.removeAllRanges(), a.addRange(s), t.style.outline = "2px dashed var(--accent-color)";
    const o = () => {
      t.contentEditable = "false", t.style.outline = "";
      const l = t.innerHTML;
      this.recordTextChange(this.selectedSelector, l), this.repositionOverlays(), t.removeEventListener("blur", o), t.removeEventListener("keydown", n);
    }, n = (l) => {
      var r, h;
      if (l.key === "Escape") {
        ...
        if (c !== void 0)
          ...
          const d = (h = v()[this.selectedSelector]) == null ? void 0 : h.text;
          d !== void 0 && (t.innerHTML = d);
        }
        t.blur();
      } else l.key === "Enter" && !l.shiftKey && (l.preventDefault(), t.blur());
    };
    t.addEventListener("blur", o), t.addEventListener("keydown", n);
  }
  recordTextChange(e, t) {
    this.draftChanges[e] || (this.draftChanges[e] = { styles: {}, text: "" }), this.draftChanges[e].text = t, y(this.draftChanges, !0);
  }
  /* --- Drag Engine --- */
  handleDragStart(e) {
    if (!this.active || !this.selectedElement) return;
    e.preventDefault();
    const t = this.selectedElement, s = window.getComputedStyle(t);
    s.position === "static" && (t.style.position = "relative", this.recordStyleChange("position", "relative")), this.dragState = {
      isDragging: !0,
      startX: e.clientX,
      startY: e.clientY,
      startLeft: parseFloat(s.left) || 0,
      startTop: parseFloat(s.top) || 0
  ...
  handleDragMove(e) {
    ...
    const t = e.clientX - this.dragState.startX, s = e.clientY - this.dragState.startY, a = this.dragState.startLeft + t, o = this.dragState.startTop + s;
    this.selectedElement.style.left = `${a}px`, this.selectedElement.style.top = `${o}px`, this.recordStyleChange("left", `${a}px`), this.recordStyleChange("top", `${o}px`), this.repositionOverlays();
  }
  handleDragEnd() {
  ...
  handleResizeStart(e) {
    if (!this.active || !this.selectedElement) return;
    e.preventDefault(), e.stopPropagation();
    const t = this.selectedElement, s = window.getComputedStyle(t), a = t.getBoundingClientRect();
    this.resizeState = {
      isResizing: !0,
  ...
  handleResizeMove(e) {
    ...
    const t = this.selectedElement, s = this.resizeState, a = e.clientX - s.startX, o = e.clientY - s.startY;
    let n = s.startWidth, l = s.startHeight, r = s.startLeft, h = s.startTop;
    const c = 10;
    if (s.handle.includes("r") && (n = Math.max(c, s.startWidth + a)), s.handle.includes("b") && (l = Math.max(c, s.startHeight + o)), s.handle.includes("l")) {
      const d = s.startWidth - a;
      d > c && (n = d, r = s.startLeft + a);
    }
    if (s.handle.includes("t")) {
      const d = s.startHeight - o;
      d > c && (l = d, h = s.startTop + o);
    }
    (s.handle.includes("r") || s.handle.includes("l")) && (t.style.width = `${n}px`, this.recordStyleChange("width", `${n}px`)), (s.handle.includes("b") || s.handle.includes("t")) && (t.style.height = `${l}px`, this.recordStyleChange("height", `${l}px`)), s.handle.includes("l") && (t.style.left = `${r}px`, this.recordStyleChange("left", `${r}px`)), s.handle.includes("t") && (t.style.top = `${h}px`, this.recordStyleChange("top", `${h}px`)), this.repositionOverlays();
  }
  handleResizeEnd() {
  ...
  handleStyleChange(e, t) {
    this.selectedElement && (this.selectedElement.style[e] = t, this.recordStyleChange(e, t), this.repositionOverlays());
  }
  recordStyleChange(e, t) {
    const s = this.selectedSelector;
    s && (this.draftChanges[s] || (this.draftChanges[s] = { styles: {}, text: void 0 }), this.draftChanges[s].styles || (this.draftChanges[s].styles = {}), this.draftChanges[s].styles[e] = t);
  }
  /* --- Save / Cancel Actions --- */
  saveChanges() {
    const e = v();
    Object.entries(this.draftChanges).forEach(([s, a]) => {
      e[s] || (e[s] = { styles: {}, text: void 0 }), a.styles && (e[s].styles = {
        ...e[s].styles,
        ...a.styles
      }), a.text !== void 0 && (e[s].text = a.text);
  ...
  cancelChanges() {
    x(this.draftChanges), this.draftChanges = {}, this.ui.showToast("Changes discarded.", "info"), this.deselectElement();
  }
  resetAllChanges() {
    z(), this.draftChanges = {}, this.ui.showToast("All visual overrides reset.", "danger"), this.deselectElement();
  }
  generateCSSExport(e) {
    let t = "";
    return Object.entries(e).forEach(([s, a]) => {
      const o = a.styles || {}, n = Object.keys(o);
      n.length > 0 && (t += `${s} {
`, n.forEach((l) => {
        const r = C(l);
        t += `  ${r}: ${o[l]};
...
function g() {
  if (!window.__CanvasEditor) {
    ...
    try {
      window.__CanvasEditor = new R(), console.log("Canvas Visual Editor initialized successfully!");
    } catch (i) {
