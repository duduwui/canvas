...
class L {
  ...
  showHover(e, t) {
    e && (this.hoverOverlay.style.display = "block", this.hoverOverlay.style.top = `${e.top}px`, this.hoverOverlay.style.left = `${e.left}px`, this.hoverOverlay.style.width = `${e.width}px`, this.hoverOverlay.style.height = `${e.height}px`, this.hoverOverlay.style.borderRadius = e.borderRadius || "0px", this.hoverOverlay.querySelector(".overlay-label").textContent = t);
  }
  hideHover() {
    this.hoverOverlay.style.display = "none";
  }
  showSelection(e, t) {
    e && (this.selectionOverlay.style.display = "block", this.selectionOverlay.style.top = `${e.top}px`, this.selectionOverlay.style.left = `${e.left}px`, this.selectionOverlay.style.width = `${e.width}px`, this.selectionOverlay.style.height = `${e.height}px`, this.selectionOverlay.style.borderRadius = e.borderRadius || "0px", this.selectionOverlay.querySelector(".overlay-label").textContent = t, this.selectionOverlay.querySelector(".dimension-label").textContent = `${Math.round(e.originalWidth)}px × ${Math.round(e.originalHeight)}px`);
  }
  ...
  showToast(e, t = "info") {
    ...
    o.className = `canvas-toast toast-${t}`, o.textContent = e, this.toastContainer.appendChild(o), setTimeout(() => {
...
class D {
  ...
  handleMouseMove(e) {
    ...
    const t = e.target, o = `${t.tagName.toLowerCase()}${t.classList.length ? "." + Array.from(t.classList).join(".") : ""}`, s = this.calculateOverlayRect(t);
    this.ui.showHover(s, o);
  }
  handleMouseClick(e) {
  ...
  selectElement(e) {
    ...
    const s = window.getComputedStyle(e), a = {
      ...
      width: e.style.width || s.width,
      height: e.style.height || s.height,
      fontSize: s.fontSize,
  ...
  repositionOverlays() {
    ...
    const e = this.calculateOverlayRect(this.selectedElement);
  ...
  recordStyleChange(e, t) {
    const o = this.selectedSelector;
    o && (this.draftChanges[o] || (this.draftChanges[o] = { styles: {}, text: void 0 }), this.draftChanges[o].styles || (this.draftChanges[o].styles = {}), this.draftChanges[o].styles[e] = t);
  }
  /* --- Save / Cancel Actions --- */
  saveChanges() {
    const e = v();
